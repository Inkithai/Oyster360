...
### Added
- Expanded backend test suite with comprehensive tests across all 24 API routers and 28 services: billing endpoints (`test_billing.py`), environment logging (`test_environment_api.py`), growth logs and timeline (`test_growth_logs_api.py`), harvests and completion stage transitions (`test_harvests_api.py`), harvest grading (`test_harvest_grades_api.py`), rooms (`test_rooms_api.py`), strains (`test_strains_api.py`), recipes (`test_recipes_api.py`), purchases (`test_purchases_api.py`), inventory movements (`test_inventory_api.py`), webhooks (`test_webhooks_api.py`), and AI assistant (`test_ai_assistant_api.py`). Backend suite now comprises 397 tests with >94% code coverage.
- Expanded frontend component and page test suite with Vitest specifications for forgot-password (`app/forgot-password/__tests__/page.test.tsx`), subscription management (`app/settings/subscription/__tests__/page.test.tsx`), strains (`app/strains/__tests__/page.test.tsx`), recipes (`app/recipes/__tests__/page.test.tsx`), harvests (`app/harvests/__tests__/page.test.tsx`), grading (`app/grading/__tests__/page.test.tsx`), and admin dashboard (`app/admin/dashboard/__tests__/page.test.tsx`). Frontend suite now includes 21 test files (61 tests) with >95% statement coverage.
- `scripts/check_test_pairing.py`: automated check enforcing that pull requests modifying production code (`backend/app/` or `frontend/src/`) include corresponding test diffs in the same PR or commit.
- `docs/database_migrations.md`: comprehensive guide covering schema versioning, migration file size limits, safe enum upgrades, autogeneration procedures, and rollback testing.
...
### Changed
- Split the monolithic 506 LOC Alembic migration into two clean, cohesive revisions: `003_add_tenant_and_operations_tables.py` and `004_add_ai_and_grading_tables.py`. Every file in the entire repository is now strictly under 330 LOC.
- Cleaned up `backend/app/main.py` middleware wiring and relocated all mid-file imports to the top level with an explicit, organized middleware pipeline (CORS -> Tenant -> Security Headers -> Request ID -> Rate Limiting).
- Machine-enforced flake8 migration exclusions across `.flake8`, `backend/.flake8`, `setup.cfg`, `backend/setup.cfg`, and `.pre-commit-config.yaml`.
- Resolved `python-json-logger` module deprecation warning by importing `pythonjsonlogger.json` with backward-compatible fallback.
- Replaced deprecated Redis `setex` invocation with modern `redis.set(key, value, ex=ttl)`.
...
- Test coverage for the GDPR compliance endpoints (Article 15 export and Article 17 erasure), image-inspection tenant boundaries, the MFA enrolment flow, the AI/assistant routers, platform analytics admin-only authorization, and farm analytics tenant scoping. Backend coverage rose to ~91% across 350 tests.
- `docs/testing.md` now documents a `coverage.py` measurement caveat: `async def` route handlers exercised through `TestClient` under-report coverage (38% vs 64% for identical assertions on `webhooks.py`), because the coroutine runs on the portal's worker thread. Reported figures for the two async routers are a lower bound, not a test gap.
...
- The Stripe webhook handler raised an uncaught `KeyError` (HTTP 500) when an event's `metadata` object omitted `organization_id` entirely, rather than returning the intended 400. Regression tests cover both the absent-key and absent-metadata cases.
